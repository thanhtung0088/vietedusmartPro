...
const Timetable: React.FC<TimetableProps> = ({ onBack }) => {
  ...
  // Load lịch từ localStorage khi mở trang
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        localStorage.removeItem('vietedu_schedule');
      }
    }
  }, []);

  // Lưu lịch vào localStorage
  const saveSchedule = (data: ScheduleItem[]) => {
    setSchedule(data);
    localStorage.setItem('vietedu_schedule', JSON.stringify(data));
    setLastUpdated(new Date().toLocaleTimeString('vi-VN'));
  };

  // Xử lý quét ảnh (tạm thời dùng dữ liệu mẫu để demo)
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Giả lập quá trình quét ảnh bằng AI
    setTimeout(() => {
      const mockData: ScheduleItem[] = [
        { period: 1, subject: 'Toán học', isMorning: true },
        { period: 2, subject: 'Ngữ văn', isMorning: true },
        { period: 3, subject: 'Sinh học', isMorning: true },
        { period: 4, subject: 'TIẾT TRỐNG', isMorning: true },
        { period: 5, subject: 'Tiếng Anh', isMorning: true },
      ];

      saveSchedule(mockData);
      setIsScanning(false);
      alert('Đã quét thành công ảnh thời khóa biểu mẫu! (Tính năng AI thật sẽ sớm có nhé 🚀)');
    }, 2000);
  };

  // Nhập thủ công dữ liệu mẫu
  const handleManualInput = () => {
    const dummy: ScheduleItem[] = [
      { period: 1, subject: 'Toán học', isMorning: true },
      { period: 2, subject: 'Vật lý', isMorning: true },
      { period: 3, subject: 'TIẾT TRỐNG', isMorning: true },
      { period: 4, subject: 'Hóa học', isMorning: true },
      { period: 5, subject: 'GDCD', isMorning: true },
    ];

    saveSchedule(dummy);
    alert('Đã nạp lịch dạy mẫu thành công!');
  };

  // Đếm tiết dạy buổi sáng
  const morningLessons = schedule.filter(s => s.isMorning && s.subject !== 'TIẾT TRỐNG').length;
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end">
        <div className="flex flex-col gap-4">
          <button
            onClick={onBack}
            className="w-fit text-sm font-bold text-slate-400 hover:text-blue-600 flex items-center gap-2 uppercase tracking-widest"
          >
          ...
          <div>
            ...
            <p className="text-[10px] text-amber-600 font-black uppercase tracking-[0.2em] mt-2 italic">
              Dữ liệu thật từ quét ảnh OCR hoặc nhập thủ công
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <label className="bg-white border-2 border-slate-200 text-slate-800 px-8 py-4 rounded-2xl font-black shadow-sm hover:border-amber-500 hover:text-amber-600 cursor-pointer transition-all flex items-center gap-4 uppercase tracking-widest text-xs">
            <input type="file" className="hidden" accept="image/*" onChange={handleFile} disabled={isScanning} />
          ...
          <button
            ...
            className="bg-[#0f172a] text-white px-10 py-4 rounded-2xl font-black shadow-2xl hover:bg-black transition-all flex items-center gap-4 uppercase tracking-widest text-xs"
          >
          ...
          <div className="flex-1 p-10 overflow-x-auto">
            {schedule.length > 0 ? (
                  ...
                  {[1, 2, 3, 4, 5].map(p => {
                    ...
                    return (
                      <tr key={p} className="group">
                        <td className="py-8 text-sm font-black text-slate-200 group-hover:text-blue-600 transition-all italic tracking-tighter">
                          0{p}
                        </td>
                        <td className="py-8 font-black text-slate-700 text-lg italic uppercase tracking-tighter">
                          {isFree ? 'TIẾT TRỐNG' : entry?.subject}
                        </td>
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-200 opacity-20 py-24">
                ...
                <p className="text-xs mt-4">Dùng nút "Quét ảnh TKB" hoặc "Nhập thủ công" để bắt đầu</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0f172a] rounded-[2.5rem] p-8 text-white shadow-xl flex flex-col justify-between h-72 relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <p className="text-4xl font-black italic tracking-tighter uppercase leading-tight">ĐANG PHÁT TRIỂN</p>
            </div>
